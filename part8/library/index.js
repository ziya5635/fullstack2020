...
const resolvers = {
  Mutation: {
    addBook: (root, args) => {
      let author = authors.find(item => item.name === args.author)
      if (!author) {
  ...
  Query: {
    ...
    allAuthors: () => {
     ...
     return res.map(item => {
      const born = authors.find(author => author.name === item[0].author).born
      return {name: item[0].author, bookCount: item.length, born: born}
     })
